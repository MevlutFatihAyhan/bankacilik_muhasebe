    ...
    BEGIN
        ...
        SELECT HESAP_NO, MUSTERI_ID, IBAN, HESAP_TURU, DOVIZ_CINSI, BAKIYE, DURUM
         ...
         WHERE (P_SEARCH_TERM IS NULL OR (
                   ...
                   UPPER(IBAN) LIKE '%' || UPPER(P_SEARCH_TERM) || '%' OR
                   UPPER(HESAP_TURU) LIKE '%' || UPPER(P_SEARCH_TERM) || '%' OR
                   TO_CHAR(MUSTERI_ID) LIKE '%' || P_SEARCH_TERM || '%'
               ))
           AND (P_HESAP_TURU IS NULL OR UPPER(HESAP_TURU) = UPPER(P_HESAP_TURU))
           AND (P_DOVIZ_CINSI IS NULL OR UPPER(DOVIZ_CINSI) = UPPER(P_DOVIZ_CINSI))
           AND (P_DURUM IS NULL OR DURUM = P_DURUM)
           AND (P_MIN_BAKIYE IS NULL OR BAKIYE >= P_MIN_BAKIYE)
           AND (P_MAX_BAKIYE IS NULL OR BAKIYE <= P_MAX_BAKIYE)
         ORDER BY HESAP_NO;
    END PRC_HESAP_LISTE;
    ...
    BEGIN
        ...
        SELECT ISLEM_ID, HESAP_NO, ISLEM_YONU, ISLEM_TUTARI, DOVIZ_CINSI, 
         ...
         WHERE (P_SEARCH_TERM IS NULL OR (
           ...
           AND (P_DOVIZ_CINSI IS NULL OR UPPER(DOVIZ_CINSI) = UPPER(P_DOVIZ_CINSI))
           AND (P_BASLANGIC_TARIHI IS NULL OR ISLEM_TARIHI >= P_BASLANGIC_TARIHI)
           AND (P_BITIS_TARIHI IS NULL OR ISLEM_TARIHI <= P_BITIS_TARIHI)
