...
BEGIN
    -- INSERT Durumu
    IF INSERTING THEN
        MERGE INTO MVD_GUNLUK_ISLEM_HACMI g
        USING (SELECT TRUNC(CAST(:NEW.ISLEM_TARIHI AS DATE)) AS H_TARIHI FROM DUAL) d
        ON (g.HACIM_TARIHI = d.H_TARIHI)
        WHEN MATCHED THEN
            UPDATE SET 
                g.ISLEM_ADEDI = g.ISLEM_ADEDI + 1,
                g.TOPLAM_MUTLAK_TUTAR = g.TOPLAM_MUTLAK_TUTAR + ABS(:NEW.ISLEM_TUTARI),
                g.SON_GUNCELLEME_TARIHI = SYSTIMESTAMP
        WHEN NOT MATCHED THEN
            INSERT (HACIM_TARIHI, ISLEM_ADEDI, TOPLAM_MUTLAK_TUTAR, SON_GUNCELLEME_TARIHI)
            VALUES (d.H_TARIHI, 1, ABS(:NEW.ISLEM_TUTARI), SYSTIMESTAMP);

    -- DELETE Durumu
    ELSIF DELETING THEN
        UPDATE MVD_GUNLUK_ISLEM_HACMI
           SET ISLEM_ADEDI = ISLEM_ADEDI - 1,
               TOPLAM_MUTLAK_TUTAR = TOPLAM_MUTLAK_TUTAR - ABS(:OLD.ISLEM_TUTARI),
               SON_GUNCELLEME_TARIHI = SYSTIMESTAMP
         WHERE HACIM_TARIHI = TRUNC(CAST(:OLD.ISLEM_TARIHI AS DATE));
         
    -- UPDATE Durumu
    ELSIF UPDATING THEN
        -- Sadece tutar veya tarih değiştiyse işlem yap
        IF :OLD.ISLEM_TUTARI != :NEW.ISLEM_TUTARI OR TRUNC(CAST(:OLD.ISLEM_TARIHI AS DATE)) != TRUNC(CAST(:NEW.ISLEM_TARIHI AS DATE)) THEN
            -- Eski değeri düş
            UPDATE MVD_GUNLUK_ISLEM_HACMI
               SET ISLEM_ADEDI = ISLEM_ADEDI - 1,
                   TOPLAM_MUTLAK_TUTAR = TOPLAM_MUTLAK_TUTAR - ABS(:OLD.ISLEM_TUTARI),
